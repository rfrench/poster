...
module.exports = (function() { 'use strict';
	...
	function upload(uploadUrl, parsedUri, uploadOptions, size, fileName, dontEscapeFields, fileContentType, callback) {
		var form = getMultipartForm(uploadOptions.fields, uploadOptions.fileId, fileName, dontEscapeFields, fileContentType);
		var contentLength = form.length + size + MULTIPART_END.length;
		var resData = '';
	...
	* on the remote web server of if maxFileSize is provided, we can bail out if it's too large */
	...
	function getMultipartForm(fields, fileFieldName, fileName, dontEscapeFields, fileContentType) {
		var form = '';
		if (fields) {
	...
	return {
		post: function(uri, options, callback) {
			...
			var dontEscapeFields = options.dontEscapeFields || false;
			var fileContentType = options.fileContentType || 'text/plain';
			...
			try {
				...
				if (parsedUri.isValidUrl) {
					head(parsedUri, uploadOptions, 0, function(err, fileUrl, fileSize, fileName) {
						...
						upload(uploadUrl, fileUrl, uploadOptions, fileSize, fileName, dontEscapeFields, fileContentType, callback);
					});
				}
				else {
					fs.exists(uri, function(exists) {
						...
						fs.stat(uri, function(err, stats) {
							...
							upload(uploadUrl, parsedUri, uploadOptions, stats.size, fileName, dontEscapeFields, fileContentType, callback);
						});
					});
				}
			}
