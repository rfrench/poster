...
module.exports = (function() { 'use strict';
	...
	function upload(uploadUrl, parsedUri, uploadOptions, size, fileName, fieldsEscape, fileContentType, callback) {
		console.log(fieldsEscape)
		var form = getMultipartForm(uploadOptions.fields, uploadOptions.fileId, fileName, fieldsEscape, fileContentType);
	...
	* on the remote web server of if maxFileSize is provided, we can bail out if it's too large */
	...
	function getMultipartForm(fields, fileFieldName, fileName, fieldsEscape, fileContentType) {
		console.log(fieldsEscape)
		var form = '';
		if (fields) {
	...
	return {
		post: function(uri, options, callback) {
			...
			var fieldsEscape = options.fieldsEscape || true;
			console.log(fieldsEscape)
			...
			try {
				...
				if (parsedUri.isValidUrl) {
					head(parsedUri, uploadOptions, 0, function(err, fileUrl, fileSize, fileName) {
						...
						upload(uploadUrl, fileUrl, uploadOptions, fileSize, fileName, fieldsEscape, fileContentType, callback);
					});
				}
				else {
					fs.exists(uri, function(exists) {
						...
						fs.stat(uri, function(err, stats) {
							...
							upload(uploadUrl, parsedUri, uploadOptions, stats.size, fileName, fieldsEscape, fileContentType, callback);
						});
					});
				}
			}
